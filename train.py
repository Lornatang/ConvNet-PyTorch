...
def main():
    ...
    convnext_model, ema_convnext_model = build_model()
    print(f"Build `{config.model_arch_name}` model successfully.")

    pixel_criterion = define_loss()
    print("Define all loss functions successfully.")

    optimizer = define_optimizer(convnext_model)
    ...
    if config.pretrained_model_weights_path:
        convnext_model, ema_convnext_model, start_epoch, best_acc1, optimizer, scheduler = load_state_dict(
            convnext_model,
    ...
    for epoch in range(start_epoch, config.epochs):
        train(convnext_model, ema_convnext_model, train_prefetcher, pixel_criterion, optimizer, epoch, scaler, writer)
        acc1 = validate(ema_convnext_model, valid_prefetcher, epoch, writer, "Valid")
        print("\n")
        ...
        save_checkpoint({"epoch": epoch + 1,
                         ...
                         "ema_state_dict": ema_convnext_model.state_dict(),
...
def build_model() -> [nn.Module, nn.Module]:
    convnext_model = model.__dict__[config.model_arch_name](num_classes=config.model_num_classes)
    convnext_model = convnext_model.to(device=config.device, memory_format=torch.channels_last)
    ...
    ema_convnext_model = AveragedModel(convnext_model, avg_fn=ema_avg)

    return convnext_model, ema_convnext_model
